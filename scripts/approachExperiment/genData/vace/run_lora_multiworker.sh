...
# 自定义输出目录
SPECIFIED_OUTPUT_PATH="output/approach_eval/LoRA/LoRA_output_vace_gen_IFT_srccodes.jsonl"
SPECIFIED_CONFIG_PATH="output/approach_eval/LoRA/LoRA_config_vace_gen_IFT_srccodes.json"
    ...
    for ban_dep in "${DEPRECATION_FLAGS[@]}"; do
        ...
        if [ $(( world_size % num_gpus_per_job )) -eq 0 ]; then
            ...
            for i in $(seq 0 $((num_gpus_per_job)) $((world_size-1))); do
                ...
                if [ -n "$SPECIFIED_OUTPUT_PATH" ]; then
                    args="$args --specified_output_path $SPECIFIED_OUTPUT_PATH"
                fi
                if [ -n "$SPECIFIED_CONFIG_PATH" ]; then
                    args="$args --specified_config_path $SPECIFIED_CONFIG_PATH"
                fi
