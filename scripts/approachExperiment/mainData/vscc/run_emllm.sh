...
datasets="VersiBCB_vscc" # 这里目前只计划输入一个
num_gpus_per_job=1 
rank_offset=0 
...
# prompt type
PROMPT_TYPE="VSCC_REVIEW"
# 是否跳过没有错误的样本
SKIP_NO_ERROR_SAMPLES=true
        ...
        do
            ...
            echo "Starting rank ${rank} with GPUs ${gpu_devices}"
                ...
                --world_size "${subworld_num}" \
                    ...
                    --useQAContext "${USE_QA_CONTEXT}" \
                    --QAContext_path "${QACacheContext_path}" \
                    --enableCachedErrorInfo "${ENABLE_CACHED_ERROR_INFO}" \
                    --errorinfos_filepath "${ERROR_INFO_FILEPATH}" \
                    --prompt_type "${PROMPT_TYPE}" \
                    --generated_target_code_path "${GENERATED_TARGET_CODE_PATH}" \
                    --skip_no_error_samples "${SKIP_NO_ERROR_SAMPLES}" \
                    &
