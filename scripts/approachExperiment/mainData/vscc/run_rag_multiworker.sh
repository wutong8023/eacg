...
# 任务列表
TASKS=("VSCC")
DEPRECATION_FLAGS=(false)
MAX_DEPENDENCY_NUMS=(10)
SKIP_GENERATION=false
GENERATED_QUERIES_FILE="data/temp/versibcb_format_queries.json"
...
# 对于doc的压缩
MAX_DOC_TOKENS=200
DOC_TRUNCATE_TOKENIZER="/datanfs4/chenrongyi/models/Llama-3.1-8B-Instruct"
# 对于API名称字符串匹配
ENABLE_API_NAME_STR_MATCH=false
# QA缓存上下文
QACacheContext_path="data/temp/inference/versibcb_vscc_contexts_staticAnalysis.jsonl"
# target_task_ids_file="scripts/target_task_id.json"



# review mode
REVIEW_ON_OUTPUT=true
GENERATED_TARGET_CODE_PATH="output/approach_eval/BASELINE/Llama-3.1-8B-Instruct/versibcb_vscc_Llama-3.1-8B-Instruct_maxdep10.jsonl"
# error_fix
ERROR_INFO_FILEPATH="data/temp/pyright_errors_vscc_new_wfstring.json"
STOP_TOKENS=('<end>' '</finalcode>')
# 对于multi-round inference
ENABLE_MULTIROUND_INFER=false
# MULTIROUND_INFER_ROUND=2
# UPDATE_ERROR_EACH_ROUND=true
# # 自定义输出路径
# OUTPUT_PATH="output/baselinefix_fstring.jsonl"
# 获取GPU使用情况并返回空闲GPU的ID


# errorfix with retrieval
# REVIEW_ON_OUTPUT=false
# # Path to the generated target code file (JSONL format)
# GENERATED_TARGET_CODE_PATH="output/approach_eval/BASELINE/Llama-3.1-8B/versibcb_vace_Llama-3.1-8B_maxdep10.jsonl"

# # Path to the error information file (JSON format)
# ERROR_INFO_FILEPATH="data/temp/combined_errors.json"

# # Path to the retrieved information file (JSON format, optional)
# RETRIEVED_INFO_FILEPATH="data/temp/retrieved_items.json"
# OUTPUT_PATH="output/baselinefix_withretrieval_v1.jsonl"
get_free_gpus() {
        ...
        for max_dependency_num in "${MAX_DEPENDENCY_NUMS[@]}"; do
            ...
            if [ $(( world_size % num_gpus_per_job )) -eq 0 ]; then
                ...
                for i in $(seq 0 $((num_gpus_per_job)) $((world_size-1))); do
                    ...
                    args="$args --stop_tokens ${STOP_TOKENS[@]}"
                    ...
                    if [ "$ENABLE_MULTIROUND_INFER" = "true" ]; then
                        args="$args --enable_multiround_infer"
                        args="$args --round $MULTIROUND_INFER_ROUND"
                        if [ "$UPDATE_ERROR_EACH_ROUND" = "true" ]; then
                            args="$args --updateErrorEachRound"
                        fi
                    fi
                    # 添加多worker参数
                    args="$args --rank $rank"
